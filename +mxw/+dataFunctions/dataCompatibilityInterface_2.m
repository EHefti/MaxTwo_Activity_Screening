classdef dataCompatibilityInterface_2 < handle
    ...
    methods
        ...
        function checkLenTimeCompatibility(this)
            if ~(isnumeric(this.dataLenTime) && (this.dataLenTime >= 0))
                errorMsg1 = sprintf('Data compatibility issue: Incorrect "dataLenTime" format. This could lead to problems later on when running analysis'); 
                errorMsg2 = sprintf('Please make sure the data length in seconds is a positive integer');
                error([errorMsg1, newline, errorMsg2]);
            end     
        end
        ...
        function checkTemperatureCompatibility(this)
            if ~isstruct(this.temperature)
                errorMsg1 = sprintf('Data compatibility issue: Incorrect "temperature" format. This could lead to problems later on when running analysis');
                errorMsg2 = sprintf('Please make sure temperature is a valid struct');
                error([errorMsg1, newline, errorMsg2]);
            end
        end
        
        function checkSpikeThresholdCompatibility(this)
            if ~(isnumeric(this.spikeThreshold) && (this.spikeThreshold >= 0))
                errorMsg1 = sprintf('Data compatibility issue: Incorrect "spikeThreshold" format. This could lead to problems later on when running analysis'); 
                errorMsg2 = sprintf('Please make sure the spikeThreshold is 0 or a positive number');
                error([errorMsg1, newline, errorMsg2]);
            end     
        end
